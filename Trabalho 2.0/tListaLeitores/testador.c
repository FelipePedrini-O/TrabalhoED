...
int main(){
    ...
    //TESTE DA LEITURA POR ARQUIVO
    printf("----- ListaLeitores2 -------\n");
    tListaLeitores *listaLeitores2 = criaListaLeitores();
    leLeitores(listaLeitores2);
    printf("Nome dos leitores: ");
    imprimeNomesListaLeitores(listaLeitores2);

    //TESTE DA AFINIDADE
    criaAfinidadesEntreLeitores(listaLeitores2);
    imprimeListaLeitores(listaLeitores2);
    //FIM DO TESTE DA AFINIDADE

    liberaLeitoresDaListaLeitores(listaLeitores2);
    liberaListaLeitores(listaLeitores2);
    printf("----- FIM DOS NOMES LISTA LEITORES 2 -----\n");
    //TESTE FIM

    // Cria leitores para teste
    tLista* gostos1 = criaLista();//Lista vazia, apenas para permitir o debug do testador
    int id; char nome[50];
    scanf("%d %[^\n]", &id, nome);
    tLeitor* leitor1 = criaLeitor(id, nome, gostos1);
    insereListaLeitores(listaLeitores, leitor1);
    imprimeListaLeitores(listaLeitores);

    tLista* gostos2 = criaLista();//Lista vazia, apenas para permitir o debug do testador
    scanf("%d %[^\n]", &id, nome);
    tLeitor* leitor2 = criaLeitor(id, nome, gostos2);
    insereListaLeitores(listaLeitores, leitor2);
    imprimeListaLeitores(listaLeitores);
    
    tLista* gostos3 = criaLista();//Lista vazia, apenas para permitir o debug do testador
    scanf("%d %[^\n]", &id, nome);
    tLeitor* leitor3 = criaLeitor(id, nome, gostos3);
    ...
    //Novo: Livros em Comum
    //Adiciona alguns livros em comum
    adicionaLivroLista(retornaLivroLista(biblioteca, "O Iluminado"), retornaListaLidos(leitor2));
    adicionaLivroLista(retornaLivroLista(biblioteca, "Orgulho e Preconceito"), retornaListaLidos(leitor2));
    adicionaLivroLista(retornaLivroLista(biblioteca, "Duna"), retornaListaLidos(leitor3));
    adicionaLivroLista(retornaLivroLista(biblioteca, "O Iluminado"), retornaListaLidos(leitor3));

    descobreLivrosComumListaLeitores(listaLeitores, retornaIdLeitor(leitor1), retornaIdLeitor(leitor2));
    descobreLivrosComumListaLeitores(listaLeitores, retornaIdLeitor(leitor3), retornaIdLeitor(leitor2));
    descobreLivrosComumListaLeitores(listaLeitores, retornaIdLeitor(leitor3), retornaIdLeitor(leitor1));
    //Fim do teste de livros em comum
    
    //Novo: Afinidades
    criaAfinidadesEntreLeitores(listaLeitores);
    //Fim do teste de afinidades
